...
public class CandidateView extends View {
  ...
  public static final int MAX_CANDIDATE_COUNT = 6;//在候选区的字符的个数
  ...
  public boolean pickHighlighted() {
	String strPicCandidate = "";
    if ((highlightIndex >= 0) && (listener != null)) {
      strPicCandidate = getCandidate(highlightIndex); 
      if(strPicCandidate.equals("")) 
    	  return false;
      else 
    	  listener.onPickCandidate(strPicCandidate);
      return true;
    }
    return false;
  }

  public boolean updateHighlight(int x, int y) {
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if((CangjieIME.RestMustOnKeyboard==0)){
	    Rect rect;
	    Paint p = new Paint();
	    p.setColor(Color.CYAN);
	    p.setStyle(Paint.Style.STROKE);
	    p.setStrokeWidth(12.75f);
	    rect = new Rect(
	    		candidateRect[CangjieIME.nLastCandidateIndex].left, candidateRect[CangjieIME.nLastCandidateIndex].top,
	    		candidateRect[CangjieIME.nLastCandidateIndex].right, 
	    		candidateRect[CangjieIME.nLastCandidateIndex].bottom
	        );
  ...
  private String getCandidate(int index) {
	Log.d("AbstractIME","getCandidate>>>>>>" + candidates);
